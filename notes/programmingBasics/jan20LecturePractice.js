...
function arrFunction(arr){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        //console.log(`Negative Array of Numbers : ${negArray}`)
        if( (arr[i] % 2 == 0) && (arr[i] >= 0)){
            posEvenArray.push(arr[i])
        }
        //console.log(`Positive Array of Numbers : ${posEvenArray}`)
        if((arr[i] % 2 != 0) && (arr[i] > 0)){
            posOddArray.push(arr[i])
        }
    }
    console.log("Negative Array", negArray, "Positive Even Array", posEvenArray, "Positive Odd Array", posOddArray, "the results array:", results)
}
arrFunction(arr01)
